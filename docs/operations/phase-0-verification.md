...
| 已构建 API 运行冒烟测试          | 本地通过                         | [`smoke-api-runtime.mjs`](../../scripts/smoke-api-runtime.mjs) 使用 test-only provider、不可达数据库和仅供启动握手的本地 Redis 探针启动构建产物，验证 liveness 200、readiness 503/down、随后 liveness 仍为 200，以及有界关闭。 |
...
| `pnpm audit --audit-level high`  | **当前阻断：0 个严重、1 个高危** | 2026-07-31 Batch 2/3 已移除 Prisma 与 Nest CLI 公告；Batch 4 的 Swagger 上游阻断及当前机器可读计数记录在 [`dependency-audit.md`](dependency-audit.md)。不得降低阈值或忽略退出码。                                              |
...
| API 就绪检查 | 历史离线冒烟仍覆盖依赖不可用时的 HTTP 503；2026-07-29 WSL2 实机验证中 PostgreSQL/PostGIS 与 Redis 就绪，`/health/live`、`/health/ready` 均为 HTTP 200。                | In review |
...
| 任务消费者   | 单元测试覆盖配置、启动失败处理、消费者错误和优雅关闭；2026-07-29 两次连接真实 Redis 连续观察 10 分钟，均保持运行且重启次数为零。                                       | In review |
| 管理后台     | 仓库冒烟测试覆盖构建后首页 HTTP 200、预期标题和根节点；尚未在浏览器中执行 React 产物，也未验证正式托管环境。                                                           | In review |
| 小程序       | 当前 Slice 1 `/wx` tree 的静态检查、官方编译、核心自动化和预览已通过；公众平台隐私声明仍独立阻断生产审核。                                                             | In review |
| 容器         | Dockerfile 静态契约验证 `USER node`；2026-07-29 WSL2 中 API/Worker 生产运行包以 `user=node`、只读根文件系统运行通过。                                                  | In review |
...
| 官方微信小程序工具（当前范围）                | 2026-07-29 当前 Slice 1 `/wx` 的 WXML/WXSS 编译、核心自动化与官方预览已完成；受控证据归档和独立复核仍待完成        | In review   |
