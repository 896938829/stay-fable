...
production. The Data Protection Owner records any approved change and the Engineering Owner verifies
deletion.
...
| Identity document                  | Collect only when necessary; keep no longer than the earliest legally permitted deletion point | Immediately delete primary storage, derived copy, index, and cache; immutable backups expire through the approved rolling window and are not modified for single-record physical deletion | Data Protection Owner and Security Owner based on recorded necessity and counsel confirmation              |
